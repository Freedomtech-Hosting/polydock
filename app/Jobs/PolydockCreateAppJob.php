...
class PolydockCreateAppJob implements ShouldQueue
{
   ...
   /**
    ...
    public function handle(Engine $polydockEngine): void
    {
        ...
        $this->polydockAppInstance->saveStatus(PolydockAppInstance::STATUS_CREATE);

        if($appEngine->createAppInstance($this->polydockAppInstance)) {
            PolydockPostCreateAppJob::dispatch($this->polydockAppInstance);
        } else {
            $this->polydockAppInstance->logError("Failed to execute create");
        }
    }
}
