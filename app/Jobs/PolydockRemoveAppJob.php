...
class PolydockRemoveAppJob implements ShouldQueue
{
   ...
   /**
    ...
    public function handle(Engine $polydockEngine): void
    {
        ...
        if($appEngine->removeAppInstance($this->polydockAppInstance)) {
            $this->polydockAppInstance->saveStatus(PolydockAppInstance::STATUS_POST_REMOVE);
            PolydockPostRemoveAppJob::dispatch($this->polydockAppInstance);
        } else {
            $this->polydockAppInstance->logError("Failed to execute remove");
        }
    }
}
